namespace scSearchContrib.Searcher
{
    ...
    internal static class QueryBuilder
    {
        ...
        public static Query BuildPartialFieldValueClause(Index index, string fieldName, string fieldValue)
        {
			...
			var fieldQuery = new QueryParser(version, fieldName.ToLowerInvariant(), index.Analyzer).Parse(fieldValue);
            return fieldQuery;
        }
        ...
        public static Query BuildFullTextQuery(string searchText, Index index)
        {
			...
			return new QueryParser(version, BuiltinFields.Content, index.Analyzer).Parse(searchText);
        }
		...
		public static Query BuildNumericRangeSearchParam(IEnumerable<NumericRangeSearchParam.NumericRangeField> ranges, Occur condition)
        {
		...
		public static Query BuildDateRangeSearchParam(IEnumerable<DateRangeSearchParam.DateRange> ranges, Occur condition)
        {
            Assert.ArgumentNotNull(ranges, "Ranges");
        ...
        public static Query BuildIdFilter(string fieldName, string ids)
        {
            ...
            {
				query.Add(BuildFieldQuery(fieldName, value), Occur.SHOULD);
            }
        ...
        private static Query BuildDateRangeQuery(DateRangeSearchParam.DateRange dateRange)
        {
			...
			return new TermRangeQuery(dateRange.FieldName, startDate, endDate, true, true
				);
        }

        private static Query BuildNumericRangeQuery(NumericRangeSearchParam.NumericRangeField range)
        {
			var lowerTerm = NumberTools.LongToString(range.Start);
			var upperTerm = NumberTools.LongToString(range.End);
			return new TermRangeQuery(range.FieldName, lowerTerm, upperTerm, true, true);
        }

		public static Query BuildMultiFieldQuery(IEnumerable<MultiFieldSearchParam.Refinement> refinements, Occur condition)
        {
