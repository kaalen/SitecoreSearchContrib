...
namespace scSearchContrib.Searcher
{
    ...
    public class QueryRunner : IDisposable
    {
        ...
        public virtual List<SkinnyItem> RunQuery(Query query, bool showAllVersions, Sort sorter, int start, int end, out int totalResults)
        {
            ...
            try
            {
                using (var context = new IndexSearchContext(Index))
                {
                    ...
                    if (sorter != null)
                    {
						var hits = context.Searcher.Search(query, null, int.MaxValue, sorter);
						searchhits = new SearchHits(hits, context.Searcher.IndexReader);
                    }
                    else
                    {
						searchhits = UsePreparedQuery ? context.Search(new PreparedQuery(query), int.MaxValue) :
															 context.Search(query, int.MaxValue);
                    }
        ...
        {
			return GetItems(parameters, QueryOccurance.Should, showAllVersions, sortField, reverse, start, end, out totalResults);
        }
