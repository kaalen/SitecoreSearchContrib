...
namespace scSearchContrib.Searcher.Utilities
{
    public class QueryTranslator
    {
		...
		public virtual Occur GetOccur(QueryOccurance condition)
        {
        ...
        protected virtual Query InternalParse(string query, string defaultField)
        {
            ...
            try
            {
				return Assert.ResultNotNull(new QueryParser(Lucene.Net.Util.Version.LUCENE_30, defaultField, _analyzer).Parse(query), "Query parser returned null reference");
            }
            catch (ParseException)
            {
				return Assert.ResultNotNull(new QueryParser(Lucene.Net.Util.Version.LUCENE_30, defaultField, _analyzer).Parse(Escape(query)), "Query parser returned null reference");
            }
        }
    }
}
