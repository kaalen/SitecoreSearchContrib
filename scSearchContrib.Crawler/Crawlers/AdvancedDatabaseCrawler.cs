...
namespace scSearchContrib.Crawler.Crawlers
{
    public class AdvancedDatabaseCrawler : DatabaseCrawler
    {
        ...
        protected virtual void ProcessFieldValue(SCField field, Document document, string value)
        {
			...
			if (indexType == Lucene.Net.Documents.Field.Index.ANALYZED)
            {
				ProcessField(document, BuiltinFields.Content, value, Lucene.Net.Documents.Field.Store.NO, Lucene.Net.Documents.Field.Index.ANALYZED);
            }
        }
        ...
        public virtual void AddDynamicFields(XmlNode configNode)
        {
			...
			var boostConfig = XmlUtil.GetAttribute("boost", configNode);
			...
			var boost = 1.0f;
			float.TryParse(boostConfig, out boost);
			dynamicField.Boost = boost;
